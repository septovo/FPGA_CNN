...
$outputRoot = Join-Path $projectRoot 'tests/build_stage3'
...
& $compiler '-mcpu=cortex-a9' '-mfpu=vfpv3' '-mfloat-abi=hard' "-specs=$spec" '-Wl,-build-id=none' "-Wl,-T,$linkerScript" "-L$bspLib" -o $elf @objects '-Wl,--start-group' -lxil -lgcc -lc '-Wl,--end-group'
if ($LASTEXITCODE -ne 0) { throw 'Link failed' }
Write-Output "STAGE3_ELF=$elf"
